...
public class Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    // discovered paths that have not yet been determined
    PriorityQueue<Path> pathQueue = new PriorityQueue<>();
    // already found shortest paths
    Hashtable<Vertex, Vertex> foundPath = new Hashtable<>();

    Path firstPath = new Path(vertices.get(start));
    pathQueue.add(firstPath);

    while (!pathQueue.isEmpty()) {
      Path comparing = pathQueue.poll();
      Vertex arriveVer = comparing.end;
      while (foundPath.containsKey(arriveVer)) {
        if (!pathQueue.isEmpty()) {
          comparing = pathQueue.poll();
          arriveVer = comparing.end;
        }
      }
      if (comparing.end.data.equals(end)) {
        return comparing;
      }

      foundPath.put(comparing.end, comparing.end);

      for (int i = 0; i < comparing.end.edgesLeaving.size(); i++) {
        Edge extend = comparing.end.edgesLeaving.get(i);
        Path holder = new Path(comparing, extend);
        pathQueue.add(holder);
      }
    }
  ...
  public static Graph dataSave() {
    Graph graph = new Graph();
    try {
      Scanner sc = new Scanner(new File(
          "/Users/henry/eclipse-workspace/Cheapest Flight Ticket Searcher/src/flightcost.txt"));
      sc.useDelimiter(",");
      if (!sc.hasNextLine()) {
        ...
        while (sc.hasNextLine()) {
          ...
          String end = sc.next();
          int cost = Integer.valueOf(sc.next());
          ...
          graph.insertEdge(end, start, cost);
          if (sc.hasNextLine()) {
            sc.nextLine();
          }
        }
  ...
  public static void main(String[] args) {
    ...
    while (whole) {
      System.out.println("--------------------------------------");
      System.out.println("Welcome to flight ticket searcher!");
      System.out.println("--------------------------------------");
      while (entry) {
        System.out.println("L: City List\nQ: Exit Program");
        System.out.print("ENTER L / Q : ");
        enter = sc.next();
        if (enter.equals("L") || enter.equals("l")) {
          ...
          System.out.println("Invalid enter. Type again");
        }
      }
      if (!whole) {
      ...
      while (true) {
        System.out.print("Pick the departure city (1 ~ 12): ");
        int departure = sc.nextInt();
        if (departure == 1) {
          departCity = "Chicago";
          System.out.println("Your departure is " + departCity);
          break;
        } else if (departure == 2) {
          departCity = "Minneapolis";
          System.out.println("Your departure is " + departCity);
          break;
        } else if (departure == 3) {
          departCity = "Detroit";
      ...
      System.out.println();
      while (true) {
        System.out.print("Pick the arrival city (1 ~ 12): ");
        int arrival = sc.nextInt();
        if (arrival == 1) {
          arriveCity = "Chicago";
          System.out.println("Your arrival is " + arriveCity);
          break;
        } else if (arrival == 2) {
          arriveCity = "Minneapolis";
          System.out.println("Your arrival is " + arriveCity);
          break;
        } else if (arrival == 3) {
          arriveCity = "Detroit";
      ...
      System.out.println();
      System.out.println("Your fligt is from " + departCity + " to " + arriveCity);
      System.out.println("Searching for the ticket...");
      System.out.println();
      System.out.println("The cheapest flight ticket from " + departCity + " to " + arriveCity
          + " is " + graph.getPathCost(departCity, arriveCity) + "$.");
      System.out.println();
      ...
      while (true) {
        ...
        if (answer.equalsIgnoreCase("Y")) {
          ...
          System.out.println("Thank you for using Flight Ticket Researcher!");
          System.out.println("------------------------------------------------------");
          whole = false;
          break;
        } else {
          System.out.println("Invalid enter. Enter again");
        }
      }
    }
  }
